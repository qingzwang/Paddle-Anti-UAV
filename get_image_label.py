...
train_info = {'images':[], 'type': 'instances', 
    'annotations':[], 
    'categories':[{"supercategory": "none", "id": 1, "name": "drone"},
                {"supercategory": "none", "id": 2, "name": "noise"}]}
val_info = {'images':[], 'type': 'instances', 
    'annotations':[], 
    'categories':[{"supercategory": "none", "id": 1, "name": "drone"},
                    {"supercategory": "none", "id": 2, "name": "noise"}]}
        ...
        while True:
            ...
            if success:
                if i % interval == 0:
                    ...
                    cv2.imwrite(os.path.join(val_img_path, img_name_draw), draw_frame)
                    
                    img_info = {'file_name': img_name, 
                        'height': float(height), 
                        'width': float(width), 
                        'id': val_img_id}
                    ann_info = {'area': float(w) * float(h), 'iscrowd': 0, 
                        'bbox': [float(x), float(y), float(w), float(h)], 
                        'category_id': category_id, 'ignore':0, 'image_id': val_img_id,
                        'id': val_img_id + 1}
            ...
            if success:
                if i % interval == 0:
                    ...
                    draw_frame = cv2.rectangle(frame, (x, y), (x + w, y + h), 
                        (0, 255, 0), 2)
                    img_name_draw = d + '_' + str(i) + 'draw.jpg'
                    cv2.imwrite(os.path.join(train_img_path, img_name_draw), draw_frame)
                    ...
                    ann_info = {'area': float(w) * float(h), 'iscrowd': 0, 
                        'bbox': [float(x), float(y), float(w), float(h)], 
                        'category_id': category_id, 'ignore':0, 'image_id': train_img_id,
                        'id': train_img_id + 1}
